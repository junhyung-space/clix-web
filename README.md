...
| 항목 | 내용 |
|------|------|
| **다운로드 URL** | `https://github.com/junhyung-space/clix-web/releases/latest/download/CLIX.zip` |
| **현재 버전** | v0.1.0 |
| **요구 사항** | macOS 14.0 Sonoma 이상 · Apple Silicon (arm64) |

웹사이트의 다운로드 버튼은 위 `latest/download` URL을 가리키므로, 새 release에 `CLIX.zip`만 포함하면 **별도 코드 수정 없이 자동으로 최신 버전**을 제공합니다.
...
```bash
# 신규 버전
gh -R junhyung-space/clix-web release create v0.2.0 /tmp/CLIX.zip \
  --title "CLIX v0.2.0" \
  --notes "릴리즈 노트"

# 기존 버전에 파일 교체
gh -R junhyung-space/clix-web release upload v0.2.0 /tmp/CLIX.zip --clobber
```

> 웹사이트 코드 변경 없이 다운로드가 즉시 최신 버전으로 전환됩니다.
