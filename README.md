...
기본 절차는 `clix` 레포의 배포 스크립트를 사용하는 것입니다.
...
기본값으로 ad-hoc `codesign` 재서명을 한 번 더 수행하므로, 완전 무서명 앱보다 "손상됨" 판정을 피하기 쉽습니다. 다른 사용자 배포 품질을 높이려면 `--codesign-identity "CLIX Local Self-Signed"` 같은 self-signed 인증서를 함께 사용하는 편이 낫습니다.
...
│   │   ├── TerminalWorkspace.astro
│   │   ├── SSHManagement.astro
│   │   ├── Download.astro
│   │   ├── SectionIconBadge.astro
│   │   └── guide/
│   │       ├── GuideSidebar.astro
│   │       └── sections/   # 가이드 섹션별 컴포넌트
│   ├── layouts/
...
새 i18n 키는 `ko.ts`에 먼저 추가하고 (Translations 타입 원본), `en.ts`에 같은 키로 영어 값을 추가합니다.

### 메인 페이지 섹션 순서

Hero → App Preview → AI Integration → Browser Automation → Terminal Workspace → SSH Management → Download

### 가이드 섹션 순서

Getting Started → Hosts & Connections → Port Forwarding → Terminal Workflow → AI Tool Usage → Browser Automation → Alerts, Recovery & Backup → Settings & Shortcuts
